...
class EventReqMsg(ReqMsg):
    def __init__(self, xml_tree):
        self.to_user_name = xml_tree.find("ToUserName").text
        self.create_time = xml_tree.find("CreateTime").text
        self.msg_type = xml_tree.find("MsgType").text
        self.event = xml_tree.find("Event").text
        self.token = xml_tree.find("Token").text
        self.open_kfid = xml_tree.find("OpenKfId").text
